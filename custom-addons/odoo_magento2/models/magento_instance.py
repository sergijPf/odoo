...
class MagentoInstance(models.Model):
    ...
    def _default_order_status(self):
        ...
        :return:
        """
        order_status = self.env.ref('odoo_magento2.pending')
    ...
    import_magento_order_status_ids = fields.Many2many('import.magento.order.status', 'magento_instance_order_status_rel',
                                                       ...
                                                       help="Select order status in which you want to import the orders"
    ...
    def synchronize_metadata(self):
        ...
        for record in self:
            ...
            self.env['magento.financial.status'].create_financial_status(record, 'not_paid')
    ...
    def import_currency(self):
        ...
        currency_obj = self.env['res.currency']
        magento_base_currency = magento_currency.get('default_display_currency_code')
    ...
    def get_magento_picking_records(self, state):
        ...
        self._cr.execute(invoice_query)
        result = self._cr.dictfetchall()
        view = self.env.ref('odoo_magento2.action_magento_stock_picking_tree').sudo().read()[0]
        if result:
            for data in result:
    ...
    def perform_operation(self, record_id):
        ...
        """
        view = self.env.ref('odoo_magento2.action_wizard_magento_instance_import_export_operations').sudo().read()[0]
