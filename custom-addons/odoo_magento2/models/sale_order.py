...
class SaleOrder(models.Model):
    ...
    def process_sales_order_creation(self, magento_instance, sales_order):
        ...
        auto_workflow, res_partners, message = self.check_sales_order_data(magento_instance, sales_order, website)
        if message:
        ...
        order_values = self.prepare_and_generate_sales_order_values(
            magento_instance, website, sales_order, res_partners, auto_workflow
        )
    ...
    def check_sales_order_data(self, instance, sales_order, website):
        ...
        if message:
            return allowed_flow, res_partners, message
        ...
        if not website.warehouse_id:
            return allowed_flow, res_partners, ("Warehouse is not set for the %s website. Please configure it first:"
        ...
        res_partners, message = self.env['res.partner'].check_customer_and_addresses_exist(
    ...
    def prepare_and_generate_sales_order_values(self, instance, website, sales_order, res_partners, auto_workflow):
        ...
        invoice_partner_id = res_partners[1].id if res_partners and res_partners[1] else False
        shipping_partner_id = res_partners[2].id if res_partners and res_partners[2] else False

        order_vals = {
            'client_order_ref': so_increment_id,
            'state': 'draft',
            'date_order': sales_order.get('created_at', False),
            'partner_id': res_partners[0].id if res_partners and res_partners[0] else False,
